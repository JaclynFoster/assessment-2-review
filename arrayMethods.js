...
const map5 = map4.map(newName => {
    return {
        name: "President " + newName.name,
        hobby: newName.hobby
    }
})
...
const oddArr = arrNum.filter(num => {
    return num % 2 === 1
})
...
const oddIndex = arrNum.filter((num, index) => {
    if(index % 2 === 1) {
        return num
    }
})
...
const stringWithA = arrString.filter(str => {
    return str.includes("a")
})
console.log("String with A:", stringWithA)
...
let newSum = 0;
theNumArr.forEach(num => newSum += num)
...
const myNewArray1 = []
map1.forEach(num => {
    myNewArray1.push(num - 1) }
)

console.log("newArray1:", myNewArray1)
...
const reduceSum = reduce1.reduce((acc, curr) => {
    return acc + curr
}, 0)
...
const plus100 = reduce1.reduce((acc, curr) => {
    return acc + curr
}, 100)
...
const newObjArr = objArr.reduce((acc, curr) => {
